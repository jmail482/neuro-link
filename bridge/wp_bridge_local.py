"""
wp_bridge_local.py â€” Zeus WP Bridge v1.5
v1.5:
- Added POST /run-command (forwards to http://127.0.0.1:8765/run-command)
- Added optional partial reads to POST /file/read with start + length
"""
...
app = FastAPI(title="Zeus WP Bridge", version="1.5.0")
...
def file_write(req: FileWriteReq):
    ...
    opened = False
    open_error = None
    if req.open_after_write:
        try:
            if req.app:
                subprocess.run(
                    ["powershell.exe", "-NoProfile", "-Command", "Start-Process", req.app, "-ArgumentList", str(p)],
